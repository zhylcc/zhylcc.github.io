...
事件的切换：需要给同一个对象绑定多个事件，而且多个事件还有**先后顺序**关系。
...
- **JS 对象和 jQuery 对象相互转换**
  ...
  - `jQuery 对象[索引]`或`jQuery对象.get(索引)`：将 jQuery 对象转为 JS 对象。
- **事件**
  - 在 jQuery 中将事件封装成了对应的方法。去掉了 JS 中的 .on 语法。
  - on(事件名称,执行的功能)：绑定事件。
  - off(事件名称)：解绑事件。
...
![](./img/表单属性选择器.png)
    ...
    补充：
    - prop是JQuery1.6才出现的，操作的是JS原生对象的属性，属性值可以是任意类型；
    - attr操作的是文档节点（Element对象）的属性，属性值只能是字符串类型，或调用toString转为字符串类型。
