...
![image-20200426174726083](./assets/image-20200426174726083.png)
...
![image-20200426180210590](./assets/image-20200426180210590.png)  

![image-20200426180348299](./assets/image-20200426180348299.png)
...
![image-20200426182959446](./assets/image-20200426182959446.png)



![image-20200426183024690](./assets/image-20200426183024690.png)
...
![image-20200426183313406](./assets/image-20200426183313406.png)
...
<!--SpringMVC提供的通用资源放行方式，可直接替代上述枚举放行-->
...
![image-20200426190932459](./assets/image-20200426190932459.png)
...
- 通过`@RequestParam`注解，将数量大于1个names参数打包成参数数组后， SpringMVC才能识别该数据格式，并判定形参类型是否为数组或集合，并按数组或集合对象的形式操作数据。  
...
![image-20200426193801059](./assets/image-20200426193801059.png)
...
*  **默认转换器**
    - `ObjectToObjectConverter` Object→时间
    - `IdToEntityConverter` Id→Entity
    - `FallbackObjectToStringConverter` Object→String  

* **SpringMVC对接收的数据进行自动类型转换，该工作通过Converter接口实现**  

  ![image-20200426194106166](./assets/image-20200426194106166.png)
...
public String showPage1() {
    System.out.println("user mvc controller is running ...");
    return "forward:page.jsp"; //等效于: return "page.jsp"
}
```
...
- 注意：
    - 页面访问地址中仍支持`/`，如：`redict:/page.jsp`  
    - 转发地址栏不改变，重定向改变
    - 转发可以访问`WEB-INF/`下的页面，而重定向不可以
...
```
  ...
  **如果未设定返回值，使用void类型，则默认使用访问路径与前后缀拼接作为页面地址**  
...
*   方式一：使用`HttpServletRequest`类型形参进行数据传递  
...
*   方式三：使用`ModelAndView`类型形参进行数据传递，将该对象作为返回值传递给调用者
  ...
  public ModelAndView showPageAndData3(ModelAndView modelAndView) {
      //ModelAndView mav = new ModelAndView();    //可以省略形参中的参数
      Book book  = new Book();
      ...
      modelAndView.setViewName("page"); //与上述方式[return "page"]对应
...
* 方式一：使用`HttpServletResponse`工具转换JSON数据返回
    ...
    public void showData1(HttpServletResponse response) throws IOException {
        response.gerWriter().print("message"); //直接响应字符串
    }
    ```

`@ResponseBody`: 返回响应内容而不是跳转页面

* 方式二：方式一的简化简化，与Servlet脱钩

    ```java
    @RequestMapping("/showData2")
    @ResponseBody
    public String showData2(HttpServletResponse response) throws IOException {
        return "message"
    }
    ```
...
* 方式二：使用SpringMVC提供的消息类型转换器，将对象与集合数据自动转换为JSON数据    
  ...
  ```
    ...
    使用SpringMVC注解驱动简化配置，替代上述手动配置  

    ```xml
    <!--开启springmvc注解驱动，对@ResponseBody的注解进行格式增强，追加其类型转换的功能，
        具体实现由添加的jackson包中MappingJackson2HttpMessageConverter进行-->
    <mvc:annotation-driven/>
    ```

    换成返回集合（`List<Book>`）也能用
...
![image-20200427162623591](./assets/image-20200427162623591.png)
...
![image-20200427164038103](./assets/image-20200427164038103.png)
...
* **拦截器VS过滤器**
  ...
  ![image-20200427164512745](./assets/image-20200427164512745.png)
...
![image-20200427164840131](./assets/image-20200427164840131.png)
...
![image-20200427171422781](./assets/image-20200427171422781.png)
...
* 上传文件过程分析  
  ...
  ![image-20200427174442939](./assets/image-20200427174442939.png)
...
![image-20200505144306747](./assets/image-20200505144306747.png)
...
![image-20200506160725709](./assets/image-20200506160725709.png)
...
![image-20200506093609697](./assets/image-20200506093609697.png)
